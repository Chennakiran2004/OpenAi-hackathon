...
interface CarbonCreditSidebarProps {
  onLogout: () => void;
}

export default function CarbonCreditSidebar({ onLogout }: CarbonCreditSidebarProps) {
  const navigate = useNavigate();
  const location = useLocation();

  const menuItems = [
    {
      path: "/carbon-credit/dashboard",
      label: "Dashboard",
      icon: <HiChartBar className="w-5 h-5" />,
    },
    {
      path: "/carbon-credit/upload-data",
      label: "Upload Govt Data",
      icon: <HiUpload className="w-5 h-5" />,
    },
    {
      path: "/carbon-credit/calculate",
      label: "Calculate Emission",
      icon: <HiTrendingUp className="w-5 h-5" />,
    },
    {
      path: "/carbon-credit/update-emission",
      label: "Update Emission",
      icon: <HiLightningBolt className="w-5 h-5" />,
    },
    {
      path: "/carbon-credit/industry",
      label: "Industry Status",
      icon: <HiDatabase className="w-5 h-5" />,
    },
    {
      path: "/carbon-credit/token-balance",
      label: "Token Balance",
  ...
  return (
    <div className="w-64 bg-white/80 backdrop-blur-md border-r border-gray-200 h-screen fixed left-0 top-0 flex flex-col shadow-sm">
      <div className="p-6 border-b border-gray-100">
        <button
          onClick={() => navigate("/carbon-credit/dashboard")}
          className="flex items-start gap-3 group"
        >
          <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 text-xl shadow-sm group-hover:scale-105 transition">
