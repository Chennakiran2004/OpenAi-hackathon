...
function sanitizeTonnes(value: number): number {
  return Number.isFinite(value) ? value : 0;
}

function formatIndianTonnes(value: number): string {
  const tonnes = sanitizeTonnes(value);
  if (tonnes < 100000) {
    return `${new Intl.NumberFormat("en-IN", {
      maximumFractionDigits: 0,
    }).format(tonnes)} tonnes`;
  }
  if (tonnes < 10000000) {
    return `${(tonnes / 100000).toFixed(2)} Lakh`;
  }
  return `${(tonnes / 10000000).toFixed(2)} Crore`;
}

function formatIndianTonnesCompact(value: number): string {
  const tonnes = sanitizeTonnes(value);
  if (tonnes < 100000) {
  ...
  if (tonnes < 10000000) {
    return `${(tonnes / 100000).toFixed(2)} L`;
  }
  return `${(tonnes / 10000000).toFixed(2)} Cr`;
}
...
export default function DemandPrediction() {
  ...
  // Prediction data
  const [predictionData, setPredictionData] = useState<PredictResponse | null>(
    null,
  );
  ...
  const forecastValues = predictionData?.prediction.forecast ?? [];
  const maxForecastTonnes =
    forecastValues.length > 0
      ? Math.max(...forecastValues.map((p) => p.predicted_demand_tonnes))
      : 0;
  const minForecastTonnes =
    forecastValues.length > 0
      ? Math.min(...forecastValues.map((p) => p.predicted_demand_tonnes))
      : 0;
          ...
          {!cropId && !predictionLoading && (
            <Card variant="elevated" className="text-center py-12">
              ...
              <p className="text-gray-600">
                Choose a crop from the dropdown above to see historical data and
                  ...
                  <div className="flex-1">
                    ...
                    {predictionData.prediction.error ? (
                      ...
                      <>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-sm font-semibold text-brand-primary">
                                  {(
                                    predictionData.prediction.confidence * 100
                                  ).toFixed(0)}
                                  %
                                </span>
                              </div>
                        ...
                        {/* Forecast Graph */}
                        {predictionData.prediction.forecast &&
                          predictionData.prediction.forecast.length > 0 && (
                            <div className="mb-6">
                                    ...
                                    axisY: {
                                      ...
                                      ...({
                                        labelFormatter: (e: {
                                          ...
                                          return formatIndianTonnesCompact(
                                            e.value,
                                          );
                                        },
                                        ...
                                        contentFormatter: (e: {
                                          ...
                                          const year =
                                            point?.x != null
                                              ? point.x
                                              : "Unknown year";
                                          const tonnes = sanitizeTonnes(
                                            point?.y ?? 0,
                                          );
                                          return `${year}: ${formatIndianTonnes(tonnes)}`;
                                        },
                                      },
                                    } as Record<string, unknown>),
                                    data: [
                                      {
                                        ...
                                        xValueFormatString: "####",
                                        dataPoints:
                                          predictionData.prediction.forecast.map(
                                            (point) => ({
                                              x: point.year,
                                              y: point.predicted_demand_tonnes,
                                        ...
                                        <div className="flex items-start gap-2">
                                          ...
                                          <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                              <span
                                                className={`text-sm font-bold ${isCurrentYear ? "text-brand-primary" : "text-gray-700"}`}
                                              >
                                                {point.year}
                                                {isCurrentYear && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                            ...
                            <div className="text-2xl font-bold text-green-900">
                              {formatIndianTonnes(maxForecastTonnes)}
                            </div>
                          ...
                          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                            ...
                            <div className="text-2xl font-bold text-blue-900">
                              {formatIndianTonnes(minForecastTonnes)}
                            </div>
                          ...
                          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                            ...
                            <div className="text-2xl font-bold text-purple-900">
                              {formatIndianTonnes(averageForecastTonnes)}
                            </div>
